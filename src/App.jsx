...
const DEFAULT_QUESTIONS = [
  ...
  },
  {
    ...
    correct:3,
    reveal:"DarkSkin Fav is more than a song. You already know. 💔"
  },
  {
  ...
  },
  {
    ...
    question:"Now that DarkSkin Fav is out, you will:",
    options:["Stream it once","Stream on repeat all day","Share it immediately","Stream on repeat AND share it 🎯"],
    correct:3,
    reveal:"DarkSkin Fav on repeat. Share it. That's the only correct answer. 💔"
  },
];
...
// ─────────────────────────────────────────────────────────────
// PRIZE SHOWCASE  — Capri-Sun + KitKat, front and centre
// ─────────────────────────────────────────────────────────────
const PrizeShowcase = ({ size = "large" }) => {
  const big = size === "large";
  return (
    <div style={{
      background:`linear-gradient(135deg, rgba(212,160,23,0.14), rgba(232,123,42,0.06))`,
      border:`1.5px solid ${C.gold}`,
      borderRadius: big ? 24 : 16,
      padding: big ? "30px 22px" : "18px 16px",
      textAlign:"center",
      boxShadow: big ? "0 0 50px rgba(212,160,23,0.15)" : "0 0 20px rgba(212,160,23,0.08)",
    }}>
      <div style={{ fontSize: big?11:9, letterSpacing:"0.2em", color:C.gold, marginBottom: big?16:10, fontWeight:800 }}>
        🏆 DARKSKIN FAV CARE PACKAGE
      </div>
      <div style={{ display:"flex", gap: big?32:18, justifyContent:"center", alignItems:"center" }}>
        <div style={{ textAlign:"center" }}>
          <div className="float" style={{ fontSize: big?72:40, lineHeight:1 }}>🧃</div>
          <div style={{ fontSize: big?13:10, color:C.cream, fontWeight:800, marginTop: big?12:6, letterSpacing:"0.06em" }}>
            ORANGE<br/>CAPRI-SUN
          </div>
        </div>
        <div style={{ fontSize: big?26:16, color:C.gold, fontWeight:300 }}>+</div>
        <div style={{ textAlign:"center" }}>
          <div className="float" style={{ fontSize: big?72:40, lineHeight:1, animationDelay:"0.4s" }}>🍫</div>
          <div style={{ fontSize: big?13:10, color:C.cream, fontWeight:800, marginTop: big?12:6, letterSpacing:"0.06em" }}>
            KITKAT<br/>CHOCOLATE
          </div>
        </div>
      </div>
      {big && <div style={{ fontSize:12, color:C.muted, marginTop:18 }}>One lucky winner gets both 🎁</div>}
    </div>
  );
};
...
const Landing = ({ cd, totalFans, onStart, onAdmin, questions }) => (
  <div style={{ minHeight:"100vh", background:C.bg, position:"relative" }}>
    ...
    {/* Visible admin login — fixed top-right, always reachable */}
    <button onClick={onAdmin}
      style={{ position:"fixed", top:16, right:16, zIndex:50, display:"flex", alignItems:"center", gap:6, background:"rgba(8,8,8,0.55)", backdropFilter:"blur(6px)", border:`1px solid ${C.border}`, color:C.cream, padding:"8px 14px", borderRadius:24, fontSize:12, fontWeight:600, cursor:"pointer", boxShadow:"0 2px 12px rgba(0,0,0,0.3)" }}>
      🔐 Admin
    </button>
    ...
    <div style={{ position:"relative", height:"100vh", display:"flex", flexDirection:"column" }}>
      ...
      <div style={{ position:"relative", zIndex:2, flex:1, display:"flex", flexDirection:"column", justifyContent:"flex-end", padding:"0 24px 52px", maxWidth:560, margin:"0 auto", width:"100%" }}>
        ...
        <div className="fadeUp" style={{ marginBottom:8 }}>
          <span style={{ fontSize:10, letterSpacing:"0.22em", color: cd.dropped?"#000":C.gold, background: cd.dropped?`linear-gradient(135deg,${C.gold},${C.orange})`:"rgba(212,160,23,0.1)", padding:"5px 14px", borderRadius:20, border:`1px solid ${C.border}`, fontWeight: cd.dropped?800:400 }}>
            {cd.dropped ? "🎉 OUT NOW" : "DROPPING FRIDAY"}
          </span>
        </div>
        ...
        <p className="fadeUp" style={{ fontSize:15, color:"rgba(245,236,215,0.68)", marginBottom:24, maxWidth:340, lineHeight:1.65, animationDelay:"0.18s" }}>
          {questions.length} questions. Nostalgia. Chaos. Memories.
          <br />{cd.dropped
            ? <>DarkSkin Fav is streaming everywhere now 🎵</>
            : <>Win the <span style={{ color:C.gold }}>DarkSkin Fav Care Package 🍊🍫</span></>}
        </p>

        {cd.dropped ? (
          <a href={STREAM_URL} target="_blank" rel="noopener noreferrer" className="fadeUp"
            style={{ animationDelay:"0.24s", display:"inline-flex", alignItems:"center", gap:8, marginBottom:26, background:"rgba(255,255,255,0.07)", border:`1px solid ${C.border}`, color:C.cream, padding:"12px 22px", borderRadius:40, fontSize:13, fontWeight:700, textDecoration:"none", width:"fit-content" }}>
            🎧 Stream DarkSkin Fav Now
          </a>
        ) : (
      ...
      <div style={{ position:"relative", zIndex:2, display:"flex", alignItems:"center", justifyContent:"center", height:"100%", padding:"0 24px" }}>
        <div style={{ textAlign:"center" }}>
          <p style={{ fontFamily:"Georgia,serif", fontStyle:"italic", fontSize:"clamp(19px,5vw,30px)", color:C.cream, marginBottom:8, lineHeight:1.3 }}>
            {cd.dropped ? '"DarkSkin Fav is OUT NOW"' : '"DarkSkin Fav drops Friday"'}
          </p>
    ...
    <div style={{ padding:"52px 24px 48px", maxWidth:500, margin:"0 auto" }}>
      ...
      <p style={{ color:"rgba(245,236,215,0.6)", lineHeight:1.85, fontSize:15, marginBottom:28 }}>
      ...
      <div style={{ padding:"0 22px", maxWidth:480, margin:"0 auto" }}>
        ...
        <div className="fadeUp" style={{ marginBottom:22, display:"flex", gap:16, padding:"18px", background:C.bgCard, border:`1px solid ${C.border}`, borderRadius:14, alignItems:"center", animationDelay:"0.18s" }}>
          ...
          <div>
            ...
            <div style={{ fontSize:13, color:C.muted, lineHeight:1.55 }}>DarkSkin Fav is out now. One song. Maximum damage.</div>
            <div style={{ fontSize:12, color:C.gold, marginTop:5 }}>@yusluvofficial_</div>
          </div>
        </div>

        {/* Prize showcase */}
        <div className="fadeUp" style={{ marginBottom:22, animationDelay:"0.2s" }}>
          <PrizeShowcase size="compact" />
        </div>
        ...
        <div className="fadeUp" style={{ display:"flex", flexDirection:"column", gap:11, animationDelay:"0.22s" }}>
          <a href={STREAM_URL} target="_blank" rel="noopener noreferrer"
            style={{ background:`linear-gradient(135deg,${C.gold},${C.orange})`, color:"#000", fontWeight:800, fontSize:15, padding:"15px", borderRadius:50, border:"none", cursor:"pointer", display:"flex", alignItems:"center", justifyContent:"center", gap:8, textDecoration:"none" }}>
            🎧 Stream DarkSkin Fav Now
          </a>
          <button onClick={onDownload}
            style={{ background:"rgba(212,160,23,0.1)", color:C.gold, fontWeight:700, fontSize:14, padding:"14px", borderRadius:50, border:`1px solid ${C.border}`, cursor:"pointer", display:"flex", alignItems:"center", justifyContent:"center", gap:8 }}>
        ...
        <p style={{ textAlign:"center", fontSize:11, color:C.mutedDk, marginTop:18, lineHeight:1.6 }}>
          Stream DarkSkin Fav now to be eligible 💔
        </p>
      </div>
    </div>
  );
      ...
      <div style={{ maxWidth:440, margin:"0 auto" }}>
        ...
        <div className="fadeUp" style={{ textAlign:"center", marginBottom:30 }}>
          ...
          <p style={{ fontSize:14, color:C.muted, lineHeight:1.65, marginBottom:18 }}>
            Winners chosen randomly from all complete entries.
          </p>
          <PrizeShowcase size="compact" />
        </div>
        ...
        <div className="fadeUp" style={{ display:"flex", flexDirection:"column", gap:14, animationDelay:"0.1s" }}>
          ...
          >
            ...
            <span style={{ fontSize:13, color:C.cream, lineHeight:1.6 }}>
              Notify me about new music from <strong style={{ color:C.gold }}>Yusluv</strong>
            </span>
          </div>
...
const Success = ({ fanInfo, score, questions, onHome }) => {
  ...
  return (
    <div style={{ minHeight:"100vh", background:C.bg, display:"flex", flexDirection:"column", alignItems:"center", justifyContent:"center", padding:"40px 24px", textAlign:"center" }}>
      ...
      <p className="fadeUp" style={{ fontSize:15, color:"rgba(245,236,215,0.6)", maxWidth:320, lineHeight:1.8, marginBottom:24, animationDelay:"0.2s" }}>
        Entry received. You'll be contacted at{" "}
        <strong style={{ color:C.gold }}>{fanInfo.email}</strong> if selected.
        <br /><br />
        Stream <strong style={{ color:C.gold }}>DarkSkin Fav</strong> now. 💔
      </p>
      <div className="fadeUp" style={{ marginBottom:28, width:"100%", maxWidth:320, animationDelay:"0.22s" }}>
        <PrizeShowcase size="compact" />
      </div>
      <div className="fadeUp" style={{ display:"flex", flexDirection:"column", gap:11, width:"100%", maxWidth:320, animationDelay:"0.24s" }}>
        <a href={STREAM_URL} target="_blank" rel="noopener noreferrer"
          style={{ background:`linear-gradient(135deg,${C.gold},${C.orange})`, color:"#000", fontWeight:800, fontSize:14, padding:"14px", borderRadius:50, border:"none", cursor:"pointer", textDecoration:"none" }}>
          🎧 Stream DarkSkin Fav Now
        </a>
        <button onClick={shareWhatsApp}
          style={{ background:"rgba(37,211,102,0.1)", color:"#25D366", fontWeight:700, fontSize:14, padding:"14px", borderRadius:50, border:"1px solid rgba(37,211,102,0.28)", cursor:"pointer" }}>
...
const Admin = ({ questions, submissions, saving, onSave, onBack }) => {
  ...
  const addOption = () => {
    setEditData(d => d.options.length >= MAX_OPTIONS ? d : { ...d, options:[...d.options, ""] });
  };

  const removeOption = i => {
    setEditData(d => {
      if (d.options.length <= MIN_OPTIONS) return d;
      const newOptions = d.options.filter((_, j) => j !== i);
      let newCorrect = d.correct;
      if (i === d.correct) newCorrect = 0;
      else if (i < d.correct) newCorrect = d.correct - 1;
      return { ...d, options:newOptions, correct:newCorrect };
    });
  };
            ...
            {(editIdx !== null || adding) && editData ? (
              <div style={{ background:C.bgCard, border:`1px solid ${C.border}`, borderRadius:16, padding:"22px", marginBottom:20 }}>
                ...
                <div style={{ display:"flex", flexDirection:"column", gap:14 }}>
                  ...
                  <div>
                    <label style={{ fontSize:11, color:C.muted, letterSpacing:"0.1em", display:"block", marginBottom:10 }}>
                      OPTIONS ({editData.options.length}/{MAX_OPTIONS}) — click circle to set correct answer
                    </label>
                    <div style={{ display:"flex", flexDirection:"column", gap:9 }}>
                      {editData.options.map((opt, i) => (
                        <div key={i} style={{ display:"flex", gap:10, alignItems:"center" }}>
                          ...
                          <input value={opt} onChange={e => setEditData(d => ({ ...d, options:d.options.map((o,j) => j===i?e.target.value:o) }))} placeholder={`Option ${optLabel(i)}`} style={{ flex:1 }} />
                          <button
                            onClick={() => removeOption(i)}
                            disabled={editData.options.length <= MIN_OPTIONS}
                            title={editData.options.length <= MIN_OPTIONS ? `Minimum ${MIN_OPTIONS} options` : "Remove this option"}
                            style={{ width:28, height:28, borderRadius:7, border:"1px solid rgba(231,76,60,0.3)", background: editData.options.length<=MIN_OPTIONS?"rgba(255,255,255,0.02)":"rgba(231,76,60,0.1)", color: editData.options.length<=MIN_OPTIONS?C.mutedDk:C.red, cursor: editData.options.length<=MIN_OPTIONS?"not-allowed":"pointer", fontSize:13, flexShrink:0, display:"flex", alignItems:"center", justifyContent:"center" }}>
                            ✕
                          </button>
                        </div>
                      ))}
                    ...
                    <button
                      onClick={addOption}
                      disabled={editData.options.length >= MAX_OPTIONS}
                      title={editData.options.length >= MAX_OPTIONS ? `Maximum ${MAX_OPTIONS} options` : "Add another option"}
                      style={{ marginTop:10, width:"100%", padding:"10px", borderRadius:8, border:`1px dashed ${editData.options.length>=MAX_OPTIONS?C.mutedDk:C.gold}`, background:"transparent", color: editData.options.length>=MAX_OPTIONS?C.mutedDk:C.gold, cursor: editData.options.length>=MAX_OPTIONS?"not-allowed":"pointer", fontSize:12, fontWeight:600 }}>
                      + Add Option
                    </button>
                  </div>
                  <div>
              ...
              {qs.map((q, i) => (
                <div key={q.id} style={{ background:C.bgCard, border:`1px solid ${C.border}`, borderRadius:12, padding:"14px 16px", display:"flex", gap:12, alignItems:"flex-start" }}>
                  ...
                  <div style={{ flex:1, minWidth:0 }}>
                    ...
                    <div style={{ fontSize:11, color:C.muted }}>✓ Option {optLabel(q.correct)} · {q.type} · {q.options.length} options</div>
                  </div>
...
const buildCard = (score, questions) => {
  ...
  coverImg.onload = () => {
    ...
    ctx.fillText("DarkSkin Fav Edition  ·  Out Now  ·  💔", 210, 504);
